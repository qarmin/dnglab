...
impl<'a> Decoder for IiqDecoder<'a> {
  fn raw_image(&self, file: &RawSource, _params: &RawDecodeParams, dummy: bool) -> Result<RawImage> {
    ...
    let mut image = match fmt {
      IiqCompression::Raw1 => return Err(RawlerError::DecoderFailed("IIQ Raw1 compression is not yet implemented".into())),
      IiqCompression::Raw2 => return Err(RawlerError::DecoderFailed("IIQ Raw2 compression is not yet implemented".into())),
